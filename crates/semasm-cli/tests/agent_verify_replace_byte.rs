//! End-to-end agent verify for the write-shape `replace_byte` harness.
...
fn run_agent_verify(source: &Path, allow_execution: bool) -> std::process::Output {
    ...
    let contract = workspace.join("fixtures/contracts/replace_byte.sem.toml");
...
fn skip_if_incomplete(stderr: &str) -> bool {
    ...
    eprintln!("skipping replace_byte agent verify e2e: {stderr}");
    true
}

#[test]
#[ignore = "requires nasm, ld, objdump, and qemu-user on PATH"]
fn agent_verify_replace_byte_allow_execution_is_verified() {
    let source = workspace_root().join("fixtures/asm/replace_byte.asm");
...
#[test]
#[ignore = "requires nasm, ld, objdump, and qemu-user on PATH"]
fn agent_verify_replace_byte_execution_denied_keeps_oracle() {
    let source = workspace_root().join("fixtures/asm/replace_byte.asm");
...
#[test]
#[ignore = "requires nasm, ld, objdump, and qemu-user on PATH"]
fn agent_verify_replace_byte_wrong_emits_behavior_failed() {
    let source = workspace_root().join("fixtures/asm/replace_byte_wrong.asm");
    let output = run_agent_verify(&source, true);
    let stderr = String::from_utf8_lossy(&output.stderr);
    if skip_if_incomplete(&stderr) {
        return;
