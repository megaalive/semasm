//! End-to-end Win64 agent verify for the pure-integer `max_usize` shape.
...
fn skip_if_incomplete(stderr: &str) -> bool {
    ...
    eprintln!("skipping win64 max_usize agent verify e2e: {stderr}");
    true
}

fn run_agent_verify(source: &Path, allow_execution: bool) -> std::process::Output {
    let workspace = workspace_root();
    let contract = workspace.join("fixtures/contracts/max_usize.sem.toml");
...
fn agent_verify_max_usize_win64_allow_execution_is_verified() {
    let source = workspace_root().join("fixtures/asm/max_usize_win64.asm");
...
#[test]
#[ignore = "requires nasm, lld-link, and native Windows host"]
fn agent_verify_max_usize_win64_execution_denied_keeps_oracle() {
    let source = workspace_root().join("fixtures/asm/max_usize_win64.asm");
...
#[test]
#[ignore = "requires nasm, lld-link, and native Windows host"]
fn agent_verify_max_usize_win64_wrong_emits_behavior_failed() {
    let source = workspace_root().join("fixtures/asm/max_usize_wrong_win64.asm");
    let output = run_agent_verify(&source, true);
    let stderr = String::from_utf8_lossy(&output.stderr);
    if skip_if_incomplete(&stderr) {
        return;
