//! End-to-end agent verify for concrete 1-byte cell store (`store_byte0`).
...
fn run_agent_verify(source: &Path, allow_execution: bool) -> std::process::Output {
    ...
    let contract = workspace.join("fixtures/contracts/store_byte0.sem.toml");
...
fn skip_if_incomplete(stderr: &str) -> bool {
    ...
    eprintln!("skipping store_byte0 agent verify e2e: {stderr}");
    true
}

#[test]
#[ignore = "requires nasm, link, and Win64 runner on PATH"]
fn agent_verify_store_byte0_allow_execution_is_verified() {
    let source = workspace_root().join("fixtures/asm/store_byte0_win64.asm");
    ...
    assert_eq!(value["behavior_oracle"]["id"], "builtin.buffer.store_byte0");
}
