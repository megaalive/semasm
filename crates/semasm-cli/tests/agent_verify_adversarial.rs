...
#[test]
#[ignore = "requires aarch64-linux-gnu-as/ld on PATH"]
fn agent_verify_stack_imbalance_aarch64_is_semantic_failed() {
    let source = workspace_root().join("fixtures/asm/count_byte_stack_imbalance_aarch64.S");
    let contract = workspace_root().join("fixtures/contracts/count_byte.sem.toml");
    let output = run_agent_verify(&source, &contract, Some("aarch64-unknown-linux-gnu"), false);
    let stderr = String::from_utf8_lossy(&output.stderr);
    if skip_if_incomplete(&stderr) {
...
#[test]
#[ignore = "requires riscv64-linux-gnu-as/ld on PATH"]
fn agent_verify_stack_imbalance_riscv64_is_semantic_failed() {
    let source = workspace_root().join("fixtures/asm/count_byte_stack_imbalance_riscv64.S");
    let contract = workspace_root().join("fixtures/contracts/count_byte.sem.toml");
    let output = run_agent_verify(
        &source,
        &contract,
...
#[test]
#[ignore = "requires aarch64-linux-gnu-as/ld on PATH"]
fn agent_verify_callee_saved_aarch64_is_semantic_failed() {
    let source = workspace_root().join("fixtures/asm/count_byte_callee_saved_aarch64.S");
    let contract = workspace_root().join("fixtures/contracts/count_byte.sem.toml");
    let output = run_agent_verify(&source, &contract, Some("aarch64-unknown-linux-gnu"), false);
    let stderr = String::from_utf8_lossy(&output.stderr);
    if skip_if_incomplete(&stderr) {
...
#[test]
#[ignore = "requires riscv64-linux-gnu-as/ld on PATH"]
fn agent_verify_callee_saved_riscv64_is_semantic_failed() {
    let source = workspace_root().join("fixtures/asm/count_byte_callee_saved_riscv64.S");
    let contract = workspace_root().join("fixtures/contracts/count_byte.sem.toml");
    let output = run_agent_verify(
        &source,
        &contract,
...
#[test]
#[ignore = "requires nasm, ld, objdump, and qemu-user on PATH"]
fn agent_verify_memcmp_callee_saved_sysv_is_semantic_failed() {
    let source = workspace_root().join("fixtures/asm/memcmp_callee_saved.asm");
    let contract = workspace_root().join("fixtures/contracts/memcmp.sem.toml");
    let output = run_agent_verify(&source, &contract, None, false);
    let stderr = String::from_utf8_lossy(&output.stderr);
    if skip_if_incomplete(&stderr) {
        return;
...
#[test]
#[ignore = "requires nasm, lld-link, and native Windows host"]
fn agent_verify_memcmp_callee_saved_win64_is_semantic_failed() {
    let source = workspace_root().join("fixtures/asm/memcmp_callee_saved_win64.asm");
    let contract = workspace_root().join("fixtures/contracts/memcmp.sem.toml");
    let output = run_agent_verify(&source, &contract, Some("x86_64-pc-windows-msvc"), false);
    let stderr = String::from_utf8_lossy(&output.stderr);
    if skip_if_incomplete(&stderr) {
...
// H4 Dx deepen: third unknown-mnemonic class (`rdtsc`) + trailing-bytes on
// `find_last_byte`. decode/lower maturity is NOT bumped.
#[test]
#[ignore = "requires nasm, ld, objdump, and qemu-user on PATH"]
fn agent_verify_unknown_insn_rdtsc_sysv_is_semantic_failed() {
    let source = workspace_root().join("fixtures/asm/count_byte_unknown_insn_rdtsc.asm");
    let contract = workspace_root().join("fixtures/contracts/count_byte.sem.toml");
    let output = run_agent_verify(&source, &contract, None, false);
    let stderr = String::from_utf8_lossy(&output.stderr);
    if skip_if_incomplete(&stderr) {
...
#[test]
#[ignore = "requires nasm, lld-link, and native Windows host"]
fn agent_verify_unknown_insn_rdtsc_win64_is_semantic_failed() {
    let source = workspace_root().join("fixtures/asm/count_byte_unknown_insn_rdtsc_win64.asm");
    let contract = workspace_root().join("fixtures/contracts/count_byte.sem.toml");
    let output = run_agent_verify(&source, &contract, Some("x86_64-pc-windows-msvc"), false);
    let stderr = String::from_utf8_lossy(&output.stderr);
    if skip_if_incomplete(&stderr) {
...
#[test]
#[ignore = "requires nasm, ld, objdump, and qemu-user on PATH"]
fn agent_verify_find_last_byte_trailing_bytes_sysv_is_semantic_failed() {
    let source = workspace_root().join("fixtures/asm/find_last_byte_trailing_bytes.asm");
    let contract = workspace_root().join("fixtures/contracts/find_last_byte.sem.toml");
    let output = run_agent_verify(&source, &contract, None, false);
    let stderr = String::from_utf8_lossy(&output.stderr);
    if skip_if_incomplete(&stderr) {
...
#[test]
#[ignore = "requires nasm, lld-link, and native Windows host"]
fn agent_verify_find_last_byte_trailing_bytes_win64_is_semantic_failed() {
    let source = workspace_root().join("fixtures/asm/find_last_byte_trailing_bytes_win64.asm");
    let contract = workspace_root().join("fixtures/contracts/find_last_byte.sem.toml");
    let output = run_agent_verify(&source, &contract, Some("x86_64-pc-windows-msvc"), false);
    let stderr = String::from_utf8_lossy(&output.stderr);
    if skip_if_incomplete(&stderr) {
...
#[test]
#[ignore = "requires aarch64-linux-gnu-as/ld on PATH"]
fn agent_verify_write_aarch64_is_semantic_failed() {
    let source = workspace_root().join("fixtures/asm/count_byte_write_aarch64.S");
    let contract = workspace_root().join("fixtures/contracts/count_byte.sem.toml");
    let output = run_agent_verify(&source, &contract, Some("aarch64-unknown-linux-gnu"), false);
    let stderr = String::from_utf8_lossy(&output.stderr);
    if skip_if_incomplete(&stderr) {
...
#[test]
#[ignore = "requires riscv64-linux-gnu-as/ld on PATH"]
fn agent_verify_write_riscv64_is_semantic_failed() {
    let source = workspace_root().join("fixtures/asm/count_byte_write_riscv64.S");
    let contract = workspace_root().join("fixtures/contracts/count_byte.sem.toml");
    let output = run_agent_verify(
        &source,
        &contract,
...
#[test]
#[ignore = "requires aarch64-linux-gnu-as/ld and qemu-aarch64 on PATH"]
fn agent_verify_replace_byte_wrong_aarch64_is_behavior_failed() {
    let source = workspace_root().join("fixtures/asm/replace_byte_wrong_aarch64.S");
    let contract = workspace_root().join("fixtures/contracts/replace_byte.sem.toml");
    let output = run_agent_verify(&source, &contract, Some("aarch64-unknown-linux-gnu"), true);
    let stderr = String::from_utf8_lossy(&output.stderr);
    if skip_if_incomplete(&stderr) {
        return;
...
#[test]
#[ignore = "requires riscv64-linux-gnu-as/ld and qemu-riscv64 on PATH"]
fn agent_verify_replace_byte_wrong_riscv64_is_behavior_failed() {
    let source = workspace_root().join("fixtures/asm/replace_byte_wrong_riscv64.S");
    let contract = workspace_root().join("fixtures/contracts/replace_byte.sem.toml");
    let output = run_agent_verify(
        &source,
        &contract,
...
#[test]
#[ignore = "requires aarch64-linux-gnu-as/ld and qemu-aarch64 on PATH"]
fn agent_verify_memset_wrong_aarch64_is_behavior_failed() {
    let source = workspace_root().join("fixtures/asm/memset_wrong_aarch64.S");
    let contract = workspace_root().join("fixtures/contracts/memset.sem.toml");
    let output = run_agent_verify(&source, &contract, Some("aarch64-unknown-linux-gnu"), true);
    let stderr = String::from_utf8_lossy(&output.stderr);
    if skip_if_incomplete(&stderr) {
        return;
...
#[test]
#[ignore = "requires riscv64-linux-gnu-as/ld and qemu-riscv64 on PATH"]
fn agent_verify_memset_wrong_riscv64_is_behavior_failed() {
    let source = workspace_root().join("fixtures/asm/memset_wrong_riscv64.S");
    let contract = workspace_root().join("fixtures/contracts/memset.sem.toml");
    let output = run_agent_verify(
        &source,
        &contract,
...
#[test]
#[ignore = "requires aarch64-linux-gnu-as/ld and qemu-aarch64 on PATH"]
fn agent_verify_memcpy_wrong_aarch64_is_behavior_failed() {
    let source = workspace_root().join("fixtures/asm/memcpy_wrong_aarch64.S");
    let contract = workspace_root().join("fixtures/contracts/memcpy.sem.toml");
    let output = run_agent_verify(&source, &contract, Some("aarch64-unknown-linux-gnu"), true);
    let stderr = String::from_utf8_lossy(&output.stderr);
    if skip_if_incomplete(&stderr) {
        return;
...
#[test]
#[ignore = "requires riscv64-linux-gnu-as/ld and qemu-riscv64 on PATH"]
fn agent_verify_memcpy_wrong_riscv64_is_behavior_failed() {
    let source = workspace_root().join("fixtures/asm/memcpy_wrong_riscv64.S");
    let contract = workspace_root().join("fixtures/contracts/memcpy.sem.toml");
    let output = run_agent_verify(
        &source,
        &contract,
...
#[test]
#[ignore = "requires aarch64-linux-gnu-as/ld and qemu-aarch64 on PATH"]
fn agent_verify_memcmp_wrong_aarch64_is_behavior_failed() {
    let source = workspace_root().join("fixtures/asm/memcmp_wrong_aarch64.S");
    let contract = workspace_root().join("fixtures/contracts/memcmp.sem.toml");
    let output = run_agent_verify(&source, &contract, Some("aarch64-unknown-linux-gnu"), true);
    let stderr = String::from_utf8_lossy(&output.stderr);
    if skip_if_incomplete(&stderr) {
...
#[test]
#[ignore = "requires riscv64-linux-gnu-as/ld and qemu-riscv64 on PATH"]
fn agent_verify_memcmp_wrong_riscv64_is_behavior_failed() {
    let source = workspace_root().join("fixtures/asm/memcmp_wrong_riscv64.S");
    let contract = workspace_root().join("fixtures/contracts/memcmp.sem.toml");
    let output = run_agent_verify(
        &source,
        &contract,
...
#[test]
#[ignore = "requires aarch64-linux-gnu-as/ld on PATH"]
fn agent_verify_memcmp_write_aarch64_is_semantic_failed() {
    let source = workspace_root().join("fixtures/asm/memcmp_write_aarch64.S");
    let contract = workspace_root().join("fixtures/contracts/memcmp.sem.toml");
    let output = run_agent_verify(&source, &contract, Some("aarch64-unknown-linux-gnu"), false);
    let stderr = String::from_utf8_lossy(&output.stderr);
    if skip_if_incomplete(&stderr) {
...
#[test]
#[ignore = "requires riscv64-linux-gnu-as/ld on PATH"]
fn agent_verify_memcmp_write_riscv64_is_semantic_failed() {
    let source = workspace_root().join("fixtures/asm/memcmp_write_riscv64.S");
    let contract = workspace_root().join("fixtures/contracts/memcmp.sem.toml");
    let output = run_agent_verify(
        &source,
        &contract,
...
#[test]
#[ignore = "requires aarch64-linux-gnu-as/ld and qemu-aarch64 on PATH"]
fn agent_verify_min_usize_wrong_aarch64_is_behavior_failed() {
    let source = workspace_root().join("fixtures/asm/min_usize_wrong_aarch64.S");
    let contract = workspace_root().join("fixtures/contracts/min_usize.sem.toml");
    let output = run_agent_verify(&source, &contract, Some("aarch64-unknown-linux-gnu"), true);
    let stderr = String::from_utf8_lossy(&output.stderr);
    if skip_if_incomplete(&stderr) {
...
#[test]
#[ignore = "requires riscv64-linux-gnu-as/ld and qemu-riscv64 on PATH"]
fn agent_verify_min_usize_wrong_riscv64_is_behavior_failed() {
    let source = workspace_root().join("fixtures/asm/min_usize_wrong_riscv64.S");
    let contract = workspace_root().join("fixtures/contracts/min_usize.sem.toml");
    let output = run_agent_verify(
        &source,
        &contract,
...
#[test]
#[ignore = "requires nasm, ld, objdump, and qemu-user on PATH"]
fn agent_verify_memcmp_write_sysv_is_semantic_failed() {
    let source = workspace_root().join("fixtures/asm/memcmp_write.asm");
    let contract = workspace_root().join("fixtures/contracts/memcmp.sem.toml");
    let output = run_agent_verify(&source, &contract, None, false);
    let stderr = String::from_utf8_lossy(&output.stderr);
    if skip_if_incomplete(&stderr) {
...
#[test]
#[ignore = "requires nasm, lld-link, and native Windows host"]
fn agent_verify_memcmp_write_win64_is_semantic_failed() {
    let source = workspace_root().join("fixtures/asm/memcmp_write_win64.asm");
    let contract = workspace_root().join("fixtures/contracts/memcmp.sem.toml");
    let output = run_agent_verify(&source, &contract, Some("x86_64-pc-windows-msvc"), false);
    let stderr = String::from_utf8_lossy(&output.stderr);
    if skip_if_incomplete(&stderr) {
