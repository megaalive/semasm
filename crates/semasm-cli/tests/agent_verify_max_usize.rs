//! End-to-end agent verify for the pure-integer `max_usize` harness shape.
...
fn skip_if_incomplete(stderr: &str) -> bool {
    ...
    eprintln!("skipping max_usize agent verify e2e: {stderr}");
    true
}

#[test]
#[ignore = "requires nasm, ld, objdump, and qemu-user on PATH"]
fn agent_verify_max_usize_allow_execution_is_verified() {
    let source = workspace_root().join("fixtures/asm/max_usize.asm");
...
#[test]
#[ignore = "requires nasm, ld, objdump, and qemu-user on PATH"]
fn agent_verify_max_usize_execution_denied_keeps_oracle() {
    let source = workspace_root().join("fixtures/asm/max_usize.asm");
...
#[test]
#[ignore = "requires nasm, ld, objdump, and qemu-user on PATH"]
fn agent_verify_max_usize_wrong_emits_behavior_failed() {
    let source = workspace_root().join("fixtures/asm/max_usize_wrong.asm");
    let output = run_agent_verify(&source, true);
    let stderr = String::from_utf8_lossy(&output.stderr);
    if skip_if_incomplete(&stderr) {
        return;
