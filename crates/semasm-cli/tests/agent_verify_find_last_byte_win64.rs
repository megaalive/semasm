//! End-to-end Win64 agent verify for the buffer `find_last_byte` shape.
...
fn skip_if_incomplete(stderr: &str) -> bool {
    ...
    eprintln!("skipping win64 find_last_byte agent verify e2e: {stderr}");
    true
}
...
fn agent_verify_find_last_byte_win64_allow_execution_is_verified() {
    let source = workspace_root().join("fixtures/asm/find_last_byte_win64.asm");
...
#[test]
#[ignore = "requires nasm, lld-link, and native Windows host"]
fn agent_verify_find_last_byte_win64_execution_denied_keeps_oracle() {
    let source = workspace_root().join("fixtures/asm/find_last_byte_win64.asm");
...
#[test]
#[ignore = "requires nasm, lld-link, and native Windows host"]
fn agent_verify_find_last_byte_win64_wrong_emits_behavior_failed() {
    let source = workspace_root().join("fixtures/asm/find_last_byte_wrong_win64.asm");
    let output = run_agent_verify(&source, true);
    let stderr = String::from_utf8_lossy(&output.stderr);
    if skip_if_incomplete(&stderr) {
        return;
